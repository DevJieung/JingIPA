...
## "이 정도면 아이가 tell 을 볼 수 있다"고 치는 문턱. **그리기와 검사기가 같은 값을 본다.**
##
## ★ 두 문턱은 서로 **같은 세기**여야 한다. 예전에는 옆으로 서는 것은 6/70(=8.6%)이면
##   읽는 것으로 쳤는데 웅크림은 0.03/0.16(=19%)을 요구했다. 그래서 tell 이 옅어지는
##   높은 탄에서 **하늘 신호만 먼저 사라졌고**(e=40 에서 웅크림이 2.4%), 검사기가
##   "가끔" 빨간불이 됐다 — 하늘이 버릇에 들어간 판에서만 났기 때문이다.
##   문턱은 예전 검사기가 쓰던 값 그대로 둔다 — 고친 것은 **웅크림의 세기**다.
##   그래서 누가 세기를 도로 낮추면 이 문턱에 걸려 탄마다 빨간불이 난다 (조용히 안 지나간다).
const TELL_DX_MIN := 6.0
const TELL_SQUASH_MAX := 0.97
