...
| 7. 자동 플레이 | `판정: 정상` · 12판이면 `클리어 1~10판`(기대 3판 안팎, 24판이면 5~9판) · **1~6탄은 크리스탈을 잃지 않는다** |
...
- **밸런스 숫자를 감으로 고치지 마라.** `core/balance.gd` 를 건드렸으면
  ...
  - **아이템에 속성을 붙이면 안 된다.** 「벼락 폭탄」을 전기로 두었더니 상성 배수를 아무리
    만져도 24판 중 0판이었다 — 후반 방어의 안전망이 돌골렘에게만 반토막 났기 때문이다.
