...
func _ready() -> void:
	...
	for i in range(6000):
		await get_tree().process_frame
		if (not b.ended) and b._settled:
			break
	print("B: ended=%s settled=%s end_t=%.2f fade=%.2f screen_is_Battle=%s"
		% [str(b.ended), str(b._settled), b.end_t, main._fade, str(main.screen == b)])
	await _save("B_after_leave")
	get_tree().quit(0)
