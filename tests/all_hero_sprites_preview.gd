...
var ids := ["chispa", "solana", "jokull", "rhiannon"]
var phase := 0.0
var units: Array[Dictionary] = []
var output := "build/all-hero-sprites/sample/1280x800"
	...
	for frame in range(14):
		if frame == 4:
			...
			for i in range(battle.sim.heroes.size()):
				...
				if nearest >= 0:
					used.append(nearest)
					battle.sim._shoot(i, nearest, 120.0, String(hero["kind"]), false)
			battle._drain()
		battle.sim._move_bullets(0.15)
		battle.sim._step_zones(0.15)
		battle._drain()
...
func _draw() -> void:
	...
	Look.text_center(self, Vector2(640, 38), "영웅 스프라이트 · Idle / Attack / Shot", 26, Look.INK)
