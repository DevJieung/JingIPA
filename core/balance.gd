...
const ELEM := {
	"none":  {"ko": "무상성", "color": "#D8DEE6"},
	"fire":  {"ko": "불",     "color": "#FF6A2A"},
	"ice":   {"ko": "얼음",   "color": "#7BDCFF"},
	"elec":  {"ko": "전기",   "color": "#FFD84D"},
	"water": {"ko": "물",     "color": "#3B8CFF"},
}
## 화면에 늘어놓을 때의 차례. 딕셔너리 순서에 기대지 않으려고 따로 둔다.
const ELEM_ORDER := ["none", "fire", "ice", "elec", "water"]

## 몬스터의 몸. 약점을 찌르면 ELEM_WEAK 배, 저항에 막히면 ELEM_RESIST 배다.
##
## ★ 표가 **대칭**이 되게 짰다 — 네 속성이 저마다 약점 셋·저항 셋을 갖는다(전기만 둘·둘).
##   그래야 어느 속성을 뽑아도 40탄 전체로는 손해도 이득도 아니고, 상성이 "운"이 아니라
##   "지금 나오는 놈이 무엇인가"의 문제가 된다. 한쪽으로 기울면 그 속성 영웅을 뽑는 순간
##   그 판이 결정돼 버린다. (검산: tests/ns_check._check_elem)
## ★ 무속성(null) 몬스터도 셋 둔다. 상성으로 풀 수 없는 놈이 있어야 순수 화력에도 뜻이 남는다.
##   30탄부터 나오는 마지막 보스가 그것이다 — 마왕은 상성으로 눕히지 못한다.
## ★ **저항은 아껴 두었다.** 열넷 중 여덟만 무언가를 튕겨 낸다 — 짐승과 마족은 약점만
##   있고 저항이 없다. 왜냐하면 자동 플레이로 재 보니 **저항 쪽이 압도적으로 세기**
##   때문이다: 약점 2배만 켜면 24판 중 11판을 깼고(안 켰을 때 5판), 저항 반감만 켜면
##   **0판**이었다. 이유는 이 게임의 모양에 있다 — 2배는 어차피 잡을 놈을 더 세게 때려
##   버려지는 몫이 크고, 반감은 **막지 못한 그 한 탄에서 크리스탈 스무 개가 통째로**
##   날아간다. 열한 마리가 저마다 무언가를 튕겨 내면 그 사고가 매 탄 도사린다.
const MBODY := {
	"null":   {"ko": "무속성", "weak": "",      "resist": ""},
	"plant":  {"ko": "식물",   "weak": "fire",  "resist": "water"},
	"aqua":   {"ko": "물",     "weak": "elec",  "resist": "fire"},
	"beast":  {"ko": "짐승",   "weak": "ice",   "resist": ""},
	"undead": {"ko": "언데드", "weak": "fire",  "resist": "ice"},
	"stone":  {"ko": "바위",   "weak": "water", "resist": "elec"},
	"flame":  {"ko": "화염",   "weak": "water", "resist": "fire"},
	"dragon": {"ko": "용",     "weak": "ice",   "resist": "fire"},
	"arcane": {"ko": "마족",   "weak": "elec",  "resist": ""},
}

## 약점을 찌르면 2배, 저항에 막히면 반이다.
##
## ★ 이 둘을 고치면 **반드시 tests/balance_check 를 다시 돌려라.**
##
## ★ 여기서 크게 한 번 헤맸으니 적어 둔다. 처음에 「벼락 폭탄」 아이템을 **전기 속성**으로
##   두었더니 저항 배수를 아무리 만져도 24판 중 0판이었다 — 0.5 도 0판, 0.65 도 1판,
##   약점을 1.5 배로 낮춰도 0판, 체력을 10% 깎아도 골드를 25% 올려도 0판. 원인은 상성이
##   아니라 **폭탄**이었다. 폭탄은 후반 방어의 안전망인데 전기로 두면 돌골렘(뒷 구간
##   체력의 3분의 1)에게 반밖에 안 들어가서, 안전망이 정확히 제일 필요한 곳에서만
##   사라진다. 폭탄을 무상성으로 되돌리자(BattleSim.use_item) 저항 0.40~0.70 이 전부
##   24판 중 13~14판으로 **거의 같아졌다** — 상성 배수 자체는 무딘 손잡이다.
## ★ 그래서 지금은 **저항을 정확히 반(0.5)으로 두고**, 상성이 올려 놓은 실력만큼을
##   처치 골드로 되돌려 놨다(kill_gold 의 주석 참고). 밸런스를 다시 잡을 일이 생기면
##   저항이 아니라 **골드**를 만져라 — 그쪽이 훨씬 곱게 듣는다.
const ELEM_WEAK := 2.0
...
static func elem_mult(attack: String, body: String) -> float:
	...
	if b.is_empty():
		return 1.0
	if String(b.get("weak", "")) == attack:
		return ELEM_WEAK
	if String(b.get("resist", "")) == attack:
		return ELEM_RESIST
	return 1.0
...
## 그 몸이 무엇에 약한가 / 무엇을 튕겨 내는가. 없으면 빈 문자열.
static func body_weak(body: String) -> String:
	return String(MBODY.get(body, {}).get("weak", ""))

static func body_resist(body: String) -> String:
	return String(MBODY.get(body, {}).get("resist", ""))
