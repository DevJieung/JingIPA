...
~/.local/bin/godot --headless --path . --export-release "iOS" "$PWD/build/ios/~~.ipa"
test -d build/ios/~~.xcodeproj || echo "!! iOS 익스포트 깨짐 — push 금지"

# (4) 안드로이드 테스트 APK
mkdir -p build/android
~/.local/bin/godot --headless --path . --export-debug "Android Test APK" \
    "$PWD/build/android/~~.apk"
cp build/android/~~.apk ~/~~.apk
...
- APK 가 어디에 생겼는지 (`~/~~.apk`, 크기)
- 내려받는 방법
  ```bash
  scp dgxmaruta@100.106.187.127:~/~~.apk .
  adb install -r ~~.apk
  ```
...
| (3) iOS | **종료 코드 0** + `build/ios/~~.xcodeproj` 생성 | 아래 3번 지뢰 점검 |
...
| `project.godot` : `config/name.ios="~~"` | 앱 표시 이름이 한글 그대로 → 무료 서명 시 **AltStore/Sideloadly 가 App ID 를 못 만듦** |
...
| `export_presets.cfg` : `application/bundle_identifier="com.example.~~"` | 저장소 Variables 의 `IOS_BUNDLE_ID` 가 덮어씁니다. 비워 두지 말 것 |
...
2. 10분쯤 뒤 실행 화면 **맨 아래 Artifacts** → `~~-ipa-N` → zip 내려받아 압축 풀기
...
| AltStore 가 앱 이름에서 오류 | 애플은 App ID 이름에 **영문·숫자만** 허용. "공룡을 찾아라!" 는 한글+띄어쓰기+`!` | `project.godot` 에 `config/name.ios="~~"` (안드로이드·PC 는 한글 그대로) |
