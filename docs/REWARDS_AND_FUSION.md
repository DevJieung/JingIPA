...
누적값이 커질수록 각 상위 등급에 도달할 확률이 증가한다. 선택한 5장의 평균 희귀도는 기존 카드 별 기준(0.5~5개)으로 계산해 별 채움과 소수 한 자리 숫자로 표시한다. 화면의 확률 설명 박스는 제거했다.

합성 결과의 등급과 관계없이 「영웅 받기」로 결과 영웅 1장을 확정하거나,
「재료 되돌리기」에서 광고 시청을 완료해 결과 영웅을 회수하고 재료 5장과 배치 위치를 복구할 수 있다.
광고 완료 후에는 복구된 재료 5장이 순서대로 펼쳐지는 애니메이션과 확인 화면을 표시한다.
...
- 치명타 확률은 단계마다 4%p 상승하며 강화 최대값은 60%(15단계)다.
- 치명타 배율은 기본 x2에서 단계마다 x0.2 상승하며 강화 최대값은 x6(20단계)다.
- 패시브·영웅 고유 효과는 기존대로 별도 적용한다. 상점 표시와 전투는 같은 공식을 사용한다.
...
| 모든 미확정 합성 결과 창 | 결과 취소 및 재료 5장 복구 |
| 게임 종료 화면 | 크리스탈 전체 복구 + 항상 1,000,000G, 메인 대기실 이동 후 같은 탄 재도전 |
...
2026-09-23 사용자 확정: 최고등급 영웅 지급은 없애고, 보유 영웅과 무관하게 항상 100만 골드를 지급한다.
실패한 탄 시작 시점의 골드로 되돌린 뒤 보너스를 한 번 더한다. 전장·전당 영웅은 추가·이동하지 않는다.
골드 획득 연출을 확인하면 능력치·패시브·배치·합성을 사용할 수 있는 메인 대기실로 이동한다.
대기실의 예고와 시작 버튼은 실패했던 같은 탄을 가리키며 손패를 다시 뽑거나 탄을 건너뛰지 않는다.
재도전을 이기면 정상적으로 다음 탄으로 진행한다. 마지막 탄의 재도전도 전투를 마쳐야 승리 처리한다.
확인 전 앱을 종료하면 다음 이어하기에서 골드 획득 화면을 다시 보여 주며 보상은 중복 지급하지 않는다.
이전 버전에서 이미 지급한 영웅의 미확인 획득 화면은 유지하되, 확인 후 메인 대기실로 이동한다.
...
- 부활 회귀 검사: `rules_check`에서 같은 탄·다음 탄의 연속 12회 부활과 매번 저장 복구·기존 영웅 보존·골드/처치 롤백 후 100만 골드 지급을 검사한다. `ads_check`에서 같은 판·같은 탄의 새 광고로 12회 반복 부활, 취소·시청 미완료·로드/표시 실패 후 재시도, 중복·이전 광고 콜백 거절을 검사한다. `revive_flow_check`에서 한영·마지막 탄 각각 3회 반복 부활의 골드 연출·대기실 확인·이어하기·강화 구매·동일 탄 재도전을 검사한다.
