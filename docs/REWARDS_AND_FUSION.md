...
게임 종료 재도전은 횟수 제한이 없다. 같은 판·같은 탄에서 다시 패배해도 매번 새 광고 시청을 완료하면 부활할 수 있다.
이전에 부활한 저장을 이어 한 경우에도 동일하다. 실패한 탄의 골드·처치 수·누적 피해는 탄 시작 시점으로 되돌린다.
...
- 부활 회귀 검사: `rules_check`에서 같은 탄·다음 탄의 연속 12회 부활과 매번 저장 복구·기존 영웅 보존·골드/처치 롤백을 검사한다. `ads_check`에서 같은 판·같은 탄의 새 광고로 12회 반복 부활, 취소·시청 미완료·로드/표시 실패 후 재시도, 중복·이전 광고 콜백 거절을 검사한다. `revive_flow_check`에서 한영 각각 3회 반복 부활의 획득 연출·확인·이어하기를 검사한다.
